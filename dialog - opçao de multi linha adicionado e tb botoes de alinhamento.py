...
PT_TRANSLATIONS = {
    ...
    # Novas traduções para o Modo Multi-linha
    "Modo Multi-linha": "Modo Multi-linha",
    "Trata todo o texto como um único card, convertendo quebras de linha em <br>": "Trata todo o texto como um único card, convertendo quebras de linha em <br>",
    # Novas traduções para alinhamento
    "Alinhar à Esquerda": "Alinhar à Esquerda",
    "Centralizar": "Centralizar",
    "Alinhar à Direita": "Alinhar à Direita",
...
class CustomDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.original_text_before_multiline = None
        self.formatted_text_in_multiline = None
    ...
    def setup_ui(self):
        ...
        self.chk_multiline_mode = QCheckBox(self._t("Modo Multi-linha"))
        self.chk_multiline_mode.setToolTip(self._t("Trata todo o texto como um único card, convertendo quebras de linha em <br>"))
        self.chk_multiline_mode.stateChanged.connect(self._on_multiline_mode_toggled)
        self.chk_multiline_mode.stateChanged.connect(self.update_card_count)
        self.chk_multiline_mode.stateChanged.connect(self.update_line_numbers)
        self.chk_multiline_mode.stateChanged.connect(self.update_preview)
        self.chk_multiline_mode.stateChanged.connect(self.schedule_save)
        options_layout.addWidget(self.chk_multiline_mode)
        ...
        self.tool_tabs.currentChanged.connect(self.schedule_save)
        ...
        for symbol, tooltip_key, align_style in align_defs:
            btn = QPushButton(symbol)
            btn.setToolTip(self._t(tooltip_key))
            btn.clicked.connect(lambda checked, align=align_style: self.apply_text_alignment(align))
            align_layout.addWidget(btn)
            self.align_buttons_widgets.append(btn)
        formatting_layout.addLayout(align_layout)
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, 'r', encoding='utf-8') as f:
                    ...
                    self.chk_multiline_mode.setChecked(dados.get('multiline_mode', False))
    ...
    def retranslate_ui(self):
        ...
        self.chk_multiline_mode.setText(self._t("Modo Multi-linha"))
        self.chk_multiline_mode.setToolTip(self._t("Trata todo o texto como um único card, convertendo quebras de linha em <br>"))
    ...
    def update_card_count(self):
        if self.chk_multiline_mode.isChecked():
            text = self.txt_entrada.toPlainText().strip()
            self.card_count_label.setText(self._t("Cards: {}").format(1 if text else 0))
            return
    ...
    def update_line_numbers(self):
        if self.chk_multiline_mode.isChecked():
            self.txt_entrada.line_number_area.line_numbers = []
            self.txt_entrada.line_number_area.update()
    ...
    def _save_in_real_time(self):
        try:
            ...
            dados = {
                ...
                'last_tab_index': self.tool_tabs.currentIndex(),
                'multiline_mode': self.chk_multiline_mode.isChecked(),
            }
    ...
    def update_preview(self):
        try:
            ...
            if self.chk_multiline_mode.isChecked():
                ...
                if ';' in full_text:
                    front_part, back_part = full_text.rsplit(';', 1)
                else:
                    front_part = full_text
                    back_part = ""
                
                front_with_br = front_part.strip().replace('\n', '<br>')
                back_with_br = back_part.strip().replace('\n', '<br>')
                
                processed_parts = [front_with_br, back_with_br]
            else:
                cursor = self.txt_entrada.textCursor()
                current_line = cursor.blockNumber()
                ...
                processed_parts = self._get_split_parts(line_text)
            ...
            if not self.chk_multiline_mode.isChecked() and self.shown_note_ids and 0 <= current_line < len(self.shown_note_ids):
                ...
                if self.chk_multiline_mode.isChecked():
                    if len(note.fields) > 0:
                        note.fields[0] = processed_parts[0]
                    if len(note.fields) > 1 and len(processed_parts) > 1:
                        note.fields[1] = processed_parts[1]
                else:
                    if not self.field_mappings:
                        for idx, field_content in enumerate(processed_parts):
                            if idx < len(note.fields):
                                note.fields[idx] = field_content.strip()
                    else:
                        field_names = [f['name'] for f in model['flds']]
                        for part_idx, field_content in enumerate(processed_parts):
    ...
    def add_cards(self):
        ...
        texto_completo = self.txt_entrada.toPlainText().strip()
        ...
        if not cloze_model:
            logging.warning("Modelo 'Cloze' não encontrado.")

        if self.chk_multiline_mode.isChecked():
            self.add_single_multiline_card(texto_completo, deck_id, default_model, cloze_model)
        else:
            self.add_cards_per_line(texto_completo, deck_id, default_model, cloze_model)

    def add_single_multiline_card(self, texto_completo, deck_id, default_model, cloze_model):
        mw.progress.start(label="Adicionando card multi-linha...", max=1)
        
        if ';' in texto_completo:
            front_part, back_part = texto_completo.rsplit(';', 1)
        else:
            front_part = texto_completo
            back_part = ""
        
        front_with_br = front_part.strip().replace('\n', '<br>')
        back_with_br = back_part.strip().replace('\n', '<br>')

        is_cloze = cloze_model and re.search(r'{{c\d+::.*?}}', front_with_br)
        
        nota = None
        if is_cloze:
            nota = mw.col.new_note(cloze_model)
            nota.fields[0] = front_with_br
            if len(nota.fields) > 1:
                nota.fields[1] = back_with_br
        else:
            nota = mw.col.new_note(default_model)
            if len(nota.fields) > 0:
                nota.fields[0] = front_with_br
            if len(nota.fields) > 1:
                nota.fields[1] = back_with_br
        
        tags_for_card = [tag.strip() for tag in self.txt_tags.toPlainText().split(',') if tag.strip()]
        if tags_for_card:
        ...
        try:
            ...
            logging.error(f"Erro ao adicionar card multi-linha: {str(e)}")
            showWarning(f"Erro ao adicionar card: {e}")

        mw.progress.finish()

        if card_added:
            showInfo(self._t("{} card adicionado com sucesso!").format(1))
        
        mw.reset()

    def add_cards_per_line(self, texto_completo, deck_id, default_model, cloze_model):
        ...
        for i, linha in enumerate(linhas):
            ...
            if is_cloze:
                ...
                nota.fields[0] = parts[0]
                if len(parts) > 1 and parts[1]:
                    nota.fields[1] = parts[1]
            else:
                nota = mw.col.new_note(default_model)
                parts = self._get_split_parts(linha)
    ...
    def show_context_menu(self, pos):
        ...
        paste_action = QAction(self._t("Colar e Formatar Questão"), self)
    ...
    def _format_text_for_multiline(self, text: str) -> str:
        """Aplica formatações de múltipla escolha e garante o ponto e vírgula."""
        ...
        for i, line in enumerate(lines):
            line = line.strip()
            if len(line) == 1 and 'A' <= line.upper() <= 'Z' and (i + 1) < len(lines):
                is_mcq = True
                break
        ...
        if is_mcq:
            ...
            while i < len(lines):
                line = lines[i].strip()
                if line.lower() == 'alternativas':
                    i += 1
                    continue
                
                if len(line) == 1 and 'A' <= line.upper() <= 'Z' and (i + 1) < len(lines):
                    option_text = lines[i + 1].strip()
                    formatted_lines.append(f"{line.upper()}) {option_text}")
                    i += 2
                else:
    ...
    def paste_html(self):
        ...
        if mime_data.hasHtml():
            ...
            text_to_process = strip_html(html_content)
        elif mime_data.hasText():
            text_to_process = clipboard.text()
        elif mime_data.hasImage():
            image = clipboard.image()
            if not image.isNull():
                media_folder = mw.col.media.dir()
                base_name, ext, counter = "img_colada", ".png", 1
                file_name = f"{base_name}{counter}{ext}"
                new_path = os.path.join(media_folder, file_name)
                while os.path.exists(new_path):
                    counter += 1
        ...
        if text_to_process:
            if self.chk_multiline_mode.isChecked():
                 formatted_text = self._format_text_for_multiline(text_to_process)
            else:
                 formatted_text = self.convert_markdown_to_html(text_to_process)
    ...
    def _on_multiline_mode_toggled(self, state):
        """Formata o texto ao entrar no modo multi-linha e reverte ao sair."""
        if state == Qt.CheckState.Checked.value:
            current_text = self.txt_entrada.toPlainText()
            self.original_text_before_multiline = current_text
            
            formatted_text = self._format_text_for_multiline(current_text)
            self.formatted_text_in_multiline = formatted_text
            
            self.txt_entrada.blockSignals(True)
            self.txt_entrada.setPlainText(formatted_text)
            self.txt_entrada.blockSignals(False)
        else:
            if self.original_text_before_multiline is not None:
                current_text = self.txt_entrada.toPlainText()
                if current_text == self.formatted_text_in_multiline:
                    self.txt_entrada.blockSignals(True)
                    self.txt_entrada.setPlainText(self.original_text_before_multiline)
                    self.txt_entrada.blockSignals(False)
    ...
    def apply_text_alignment(self, alignment: str):
        """Aplica o alinhamento de texto envolvendo o texto com tags HTML de div."""
        cursor = self.txt_entrada.textCursor()
        start_tag = f'<div style="text-align:{alignment}">'
        end_tag = '</div>'

        if cursor.hasSelection():
            texto = cursor.selectedText()
            cursor.insertText(f"{start_tag}{texto}{end_tag}")
        else:
